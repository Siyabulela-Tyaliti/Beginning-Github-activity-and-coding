...
\(n \in \mathds{N}, n > 0, \text{\hspace{0.1cm} and \hspace{0.1cm}} a \in \mathds{Z}^{+} \text{\hspace{0.1cm} such \hspace{0.1cm} that \hspace{0.1cm}} b^{n} \neq a, \forall b \in \mathds{Q} \implies \sqrt[n]{a} \in \mathds{R}^{*}/\mathds{Q}\).  

\begin{conjecture*}
  If \(n \in \mathds{N}, n > 1\) and \(a \in \mathds{Z}^{+}\) such that \(a \neq  b^n\) for all \(b \in \mathds{Z}\), then \(\sqrt[n]{a} \in \mathds{R}^{*}/\mathds{Q}\). 
\end{conjecture*} 
...
\begin{proof}
    ...
    \begin{align*}
        \sqrt[n]{a} &= \frac{L}{P} \\ 
                    ...
                    &= \displaystyle\frac{L^n}{P^n} 
    \end{align*} 
    
    If this were true, then we would obtain something that violates the condition that \(a \neq b^n\) for some integer $b$. Since \(\frac{L}{P} \in \mathds{Q}\), this then would give us a contradiction. But since we do not yet know that $\left(\frac{L}{P}\right)^n$ is not an integer, we will move forward with the proof. This then leads me to the next part of the conversation regarding this proof... 
...
\section*{A conundrum in my attempt at a proof} 

So it turned out over the course of my research that the proof I initially thought of had an error. For the result above to be fully proven, I would need to then prove that the following result is true... \\ 

\begin{lemma*}
    If \(a \neq b^n\), then \(\sqrt[n]{a} \neq \frac{L}{P}\)
\end{lemma*} 

In my attempt to prove this result I tried to prove the lemma using prove by contradiction. The outline of how I thought about it can be seen below \\ 

Assume to the contrary that $\sqrt[n]{a} = \frac{L}{P}$ where $\frac{L}{P}$ is written in its lowest terms (meaning \(L\) and \(P\) are relatively prime), we see that \\ 

\begin{align*}
    \sqrt[n]{a} &= \frac{L}{P} \\ 
    \left(\sqrt[n]{a}\right)^n &= \left(\frac{L}{P}\right)^n \\ 
    a &= \displaystyle\frac{L^n}{P^n} \\ 
    \left(P^n\right)a &= L^n 
\end{align*} 

since \(P^n \in \mathds{Z}\) that means \(a|L^n\). 

This was where I actually got stuck. \\ 

Throughout my conversations with my professor on how to proceed with the problem, he suggested that we can try to show that since \(L^n\) contains a factor of \(a\), \(P^n\) also contains a factor of \(a\), and if \(L^n\) and \(P^n\) both contain factors of \(a\), then this would contradict the assumption that \(\displaystyle\frac{L^n}{P^n}\) is written in its lowest terms. For this to be true, however, we would need a lemma or a theorem that behaves just like \hyperlink{Theorem 3.12}{Theorem 3.12}, but for all natural numbers in general and for all integers in general, not just the number \(2\). In other words, we would need to show that the lemma below is true... \\ 

\begin{lemma*}
    \(L^n\) contains a factor of \(a\) if and only if \(L\) contains a factor of \(a\)
\end{lemma*} 

My professor suggested that we could attempt to prove this lemma using prime factorization, we should consider cases where \(a\) is a prime number and we should also consider case where \(a\) is a composite number. \\ 

At the moment, I am honestly not sure how to go about doing that. \\ 
...
\paragraph{How I attempted to think about it in comparison to other people}
I have noticed throughout my research that in other texts that I have looked at so far, other people have mostly solved this problem under the domain restriction of the \(n\)th root function where \(a\) is a strictly positive integer. In most texts, people have started out by proving the irrationality of \(\sqrt{2}\) (beginning their proofs with the case where \(n = 2\) and eventually generalized that argument to other natural number greater than \(2\). The way I attempted to approach the problem is through also used a proof by contradiction, however, in my proof I attempted to start with the case where \(\sqrt[n]{a}\) is a rational number and then derive a contradiction from that case. 
